...
data= np.loadtxt('../results/sabc_calibration_clonogenic.dat', dtype='float')
...
if np.sum(all_discr)>0:
    np.savetxt('after_discriminators_clonogenic.dat', ([alpha[all_discr==1], cr[all_discr==1], ce[all_discr==1], mu_g[all_discr==1], gamma[all_discr==1]]))


#Save results of the analysis
file = open("analysis_clonogenic.txt", "w")
...
np.savetxt('parameters/parameters_min_error_clon.dat',([alpha[np.argmin(error_clon)], cr[np.argmin(error_clon)], ce[np.argmin(error_clon)], mu_g[np.argmin(error_clon)], gamma[np.argmin(error_clon)], error_clon[np.argmin(error_clon)], error_comet[np.argmin(error_clon)]]))   

alpha = alpha[np.argmin(error_clon)]
cr    = cr[np.argmin(error_clon)]
ce    = ce[np.argmin(error_clon)]
mu_g  = mu_g[np.argmin(error_clon)]
gamma = gamma[np.argmin(error_clon)]
...
for j in range(0,len(D)):
    ...
    if D[j]==6.:
        ...
        np.savetxt('synthetic/synthetic_comet_min_error_clon.txt',([comet15, comet30, comet60, comet120, comet240, comet360]))
                       
    np.savetxt('synthetic/synthetic_survival_min_error_clon.txt',np.transpose([D, survival]), header="dose S", comments="")                 
