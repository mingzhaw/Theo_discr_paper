...
data= np.loadtxt('../results/sabc_calibration_combined_1.dat', dtype='float')
...
if np.sum(all_discr)>0:
    np.savetxt('after_discriminators_combined_1.dat', ([alpha[all_discr==1], cr[all_discr==1], ce[all_discr==1], mu_g[all_discr==1], gamma[all_discr==1]]))


#Save results of the analysis
file = open("analysis_combined_1.txt", "w")
...
error = error_clon + error_comet
np.savetxt('parameters/parameters_min_error_combined_1.dat',([alpha[np.argmin(error)], cr[np.argmin(error)], ce[np.argmin(error)], mu_g[np.argmin(error)], gamma[np.argmin(error)], error_clon[np.argmin(error)], error_comet[np.argmin(error)]]))   
...
for j in range(0,len(D)):
    ...
    if D[j]==6.:
        ...
        np.savetxt('synthetic/synthetic_comet_min_error_combined_1.txt',([comet15, comet30, comet60, comet120, comet240, comet360]))
                       
np.savetxt('synthetic/synthetic_survival_min_error_combined_1.txt',np.transpose([D, survival]), header="dose S", comments="")                                    
